...
private let loginShellBootstrapCommand = """
motd_shown=0
if [ -r /run/motd.dynamic ]; then
    cat /run/motd.dynamic
    motd_shown=1
elif command -v run-parts >/dev/null 2>&1 && [ -d /etc/update-motd.d ]; then
...
if [ "$motd_shown" = 1 ]; then
    printf '\\r\\n'
fi
shell=${SHELL:-/bin/sh}
case "$shell" in
    */bash|*/zsh|*/ksh|*/fish) exec "$shell" -l ;;
    *) exec "$shell" ;;
esac
"""
